...
import { Calendar, User, Phone, Mail, Car, Clock, Search } from 'lucide-react';

const ServicePortal = () => {
    // 1. STATE: Kept from your code (with default demo value)
    const [regNumber, setRegNumber] = useState("TN-01-AB-1234");
    const [history, setHistory] = useState(null); // Initialize as null to show "Enter Reg" state initially
    const [loading, setLoading] = useState(false);

    // 2. LOGIC: Your backend integration
    const fetchHistory = async () => {
        if (!regNumber) return;

        setLoading(true);
        try {
            // EXACT BACKEND CALL PRESERVED
            const res = await axios.get(`http://127.0.0.1:8000/api/service-history/${regNumber}`);
            setHistory(res.data);
        } catch (err) {
            console.error("Failed to fetch history", err);
            setHistory([]); // Set empty array on error so UI shows "No records"
        }
        setLoading(false);
    };
        ...
        <div className="min-h-screen bg-white font-sans text-gray-800">
            ...
            <main className="max-w-6xl mx-auto px-6 py-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {/* THE MAGIC FIELD (Registration Number) - Connected to your Backend */}
                                <div className="group relative">
                                    ...
                                    <div className="flex items-center border-b border-gray-300 py-2 group-focus-within:border-red-600 transition">
                                        ...
                                        />
                                        {/* The "View Slots" Button - Triggers Axios Call */}
                                        <button
                                            onClick={fetchHistory}
                                            disabled={loading}
                                            className="bg-red-50 hover:bg-red-100 text-red-600 px-3 py-1 rounded text-xs font-bold transition flex items-center gap-2 disabled:opacity-50"
                                        >
                                            <Search size={12} />
                    ...
                    {/* RIGHT: THE HISTORY PANEL (Visuals by Friend, Data by You) */}
                                ...
                                <div className="space-y-4">
                                    {history.map((slot, index) => (
                                        <div key={index} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm relative overflow-hidden">
                                            {/* Status Color Logic based on your Backend Uppercase keys */}
                                            <div className={`absolute top-0 left-0 w-1 h-full ${slot.Status === 'COMPLETED' ? 'bg-green-500' : 'bg-blue-500'}`}></div>

                                            <div className="flex justify-between items-start mb-2">
                                                {/* Backend doesn't give Service Name, using VehicleReg or Fallback */}
                                            ...
                                            <div className="text-xs text-gray-500 flex items-center gap-4">
                                                {/* Using Capitalized Keys from your Backend (slot.Date, slot.Time) */}
